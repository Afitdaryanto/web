...
Unjail Validator
```
nolusd tx slashing unjail --from <wallet> --chain-id --chain-id pirin-1 --gas-adjustment 1.4 --gas="auto" --fees=2500unls -y
```
...
```
nolusd tx bank send wallet <TO_WALLET_ADDRESS> 1000000unls --from wallet --chain-id pirin-1
```
...
You can change value `CUSTOM_PORT=29` To any other ports
```
CUSTOM_PORT=18
